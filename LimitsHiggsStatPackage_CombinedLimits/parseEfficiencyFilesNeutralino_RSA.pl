...
if ( $inAcceptance eq 1 ) {
  $infileStem = "InputData2012_RSA//EfficiencyFiles/efficiencies_coll_Chi0ToNuLL";
} elsif ( $inAcceptance eq 2 ) {
  $infileStem = "InputData2012_RSA//EfficiencyFiles/efficiencyOverAcceptance/efficienciesOverAcceptance_coll_Chi0ToNuLL";
} else {
...
my %signalEffiSys_rel = (# "e1"  => 0.095,
                         #"mu1" => 0.248);  # with trigger
                         "mu1" => 0.181);  # without trigger
...
my %nominal_lifetimes = ("120_48" => 165.,
		         "350_148" => 173.,
		         "1000_148" => 60.,
		         "1500_494" => 160.);
...
my $ianfile;
if ($inAcceptance == 1) {
  $ianfile = "SignalSampleInfoEffi_Neutralino_RSA.py";
} elsif ($inAcceptance == 2) {
  $ianfile = "SignalSampleInfoEffiOverAcc_Neutralino_RSA.py";
}
...
foreach my $hmass (sort {$b <=> $a} keys %masses) {
	...
	next if (!$nominal_lifetimes{$pat});
        ...
        print IANFILE "#\n";
        print IANFILE "s = SignalSampleInfoRSA(${hmass}, ${xmass}, ${nominal_lifetimes{$pat}}, $inAcceptance)\n";

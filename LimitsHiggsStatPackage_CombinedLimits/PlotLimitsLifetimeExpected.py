			...
			if smooth and relerr > 0.2:
				...
				if lastInd >= 0 and nextInd < len(vecLims):
					...
					if lastScale == 0.1 or lastScale == 1.0:
						...
						if (up_br0 > max(lastLims.up_br0, nextLims.up_br0) or up_br0 < min(lastLims.up_br0, nextLims.up_br0)): 
							print "SMOOTHING up_br0",mh,mx,lt
							up_br0    = 0.5*up_br0    + 0.25*(lastLims.up_br0 + nextLims.up_br0)
						if (up_br1 > max(lastLims.up_br1, nextLims.up_br1) or up_br1 < min(lastLims.up_br1, nextLims.up_br1)): 
							print "SMOOTHING up_br1",mh,mx,lt
							up_br1    = 0.5*up_br1    + 0.25*(lastLims.up_br1 + nextLims.up_br1)
						if (est1p_br0 > max(lastLims.est1p_br0, nextLims.est1p_br0) or est1p_br0 < min(lastLims.est1p_br0, nextLims.est1p_br0)): 
							print "SMOOTHING est1p_br0",mh,mx,lt
							est1p_br0 = 0.5*est1p_br0 + 0.25*(lastLims.est1p_br0 + nextLims.est1p_br0)
						if (est1m_br0 > max(lastLims.est1m_br0, nextLims.est1m_br0) or est1m_br0 < min(lastLims.est1m_br0, nextLims.est1m_br0)): 
							print "SMOOTHING est1m_br0",mh,mx,lt
							est1m_br0 = 0.5*est1m_br0 + 0.25*(lastLims.est1m_br0 + nextLims.est1m_br0)
						if (est2p_br0 > max(lastLims.est2p_br0, nextLims.est2p_br0) or est2p_br0 < min(lastLims.est2p_br0, nextLims.est2p_br0)): 
							print "SMOOTHING est2p_br0",mh,mx,lt						 
							est2p_br0 = 0.5*est2p_br0 + 0.25*(lastLims.est2p_br0 + nextLims.est2p_br0)
						if (est2m_br0 > max(lastLims.est2m_br0, nextLims.est2m_br0) or est2m_br0 < min(lastLims.est2m_br0, nextLims.est2m_br0)): 
...
#			ltLast = lt
			...
			print "PLOTTING ",filename," mh=",plots.hmass," tau=",lt," obs=",up_br0," est=",est_br0, " (1) ",est1p_br0, "/", est1m_br0, " (2) ",est2p_br0, "/", est2m_br0,"\n"

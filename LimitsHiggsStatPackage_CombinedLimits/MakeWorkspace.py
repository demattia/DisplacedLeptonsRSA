...
def MakeWorkspace(signalInfo, LeptonType, tIndex, dump=True, plot=True):
    ...
    width_vec = signalInfo.widths[LeptonType]
    if jobOpt.loopOverCTau:
        ...
        width_use = signalInfo.getMedianWidth(LeptonType)
        print "DEBUG WIDTH ", width_use, "\n"
    ...
    if signalInfo.backgrPdfOption == 1 or signalInfo.backgrPdfOption == 2:
        # Represent Z0 peak with a BW. (not done for option 3)
    ...
    if signalInfo.backgrPdfOption == 1:
        ...
        backgrOther = r.RooGenericPdf("backgrOther", "(TMath::Erf((mass-%f)/%f)+1)/(mass*mass*mass)" % (
            signalInfo.paramTurnOn[LeptonType], signalInfo.paramTurnOnWidth[LeptonType]), argListMass)
        w.Import(backgrOther)

    elif signalInfo.backgrPdfOption == 3:
        inputDataRootFile = jobOpt.getDataRootFile(LeptonType)
        bkgFile = r.TFile(inputDataRootFile)
        bkgHist = bkgFile.Get("Data")
        bkgRooHist = r.RooDataHist("bkgRooHist", "bkgRooHist", argListMass, bkgHist)
        backgrOther = r.RooHistPdf("backgrOther", "backgrOther", w.argSet("mass"), bkgRooHist)
        w.Import(backgrOther)
    ...
    # IRT - commented out systematic uncertainty on width to save CPU.
    ...
    w.factory("width[%f]" % (width_use));  # Assumed width of signal resonance, allowing for systematic uncertainty
    ...
    if (useRealData):
        ...
        inputDataTxtFile = jobOpt.getDataTxtFile(LeptonType)
        if os.path.exists(inputDataTxtFile):
            dataAll = r.RooDataSet.read(inputDataTxtFile, argListFile)
            ...
            sys.exit("ERROR: %s does not exist" % inputDataTxtFile)
        ...
        if signalInfo.backgrPdfOption == 1 or signalInfo.backgrPdfOption == 2:
            ...
            w.factory("SUM::model_splusb(nS*signal, nB_Other*backgrOther)")
            if w.var("nS").getVal() + w.var("nB_Other").getVal() > 0:
                data = w.pdf("model_splusb").generate(w.argSet("mass"), 0)
            else:
                # Create empty dataset
                data = r.RooDataSet("empty", "", w.argSet("mass"))

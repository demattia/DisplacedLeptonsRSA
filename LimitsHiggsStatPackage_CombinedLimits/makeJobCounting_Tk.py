...
from SignalSampleInfo_Tk import *     # Includes JobOptions class.
import sys
import os
import subprocess
...
if acceptanceOption == 1:
    if jobOpt.doNeutralinoLimits:
        jobDirName = "counting_job_Tk_%s_neutralino"              %(jobOpt.limitMethod)
        jobOutName = "counting_job_output_Tk_%s_neutralino"       %(jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_Tk_%s"              %(jobOpt.limitMethod)
        jobOutName = "counting_job_output_Tk_%s"       %(jobOpt.limitMethod)
        
elif acceptanceOption == 2:
    if jobOpt.doNeutralinoLimits:
        jobDirName = "counting_job_acc_Tk_%s_neutralino"          %(jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_Tk_%s_neutralino"   %(jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_acc_Tk_%s"          %(jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_Tk_%s"   %(jobOpt.limitMethod)
else:
   ...
   if s.acceptance == acceptanceOption:
     ...
     for l in leptons:
         ...
         for tIndex, t in enumerate(s.cTauScaleLoopValues):
               ...
               cmd  = 'cp combine_template_counting_Tk.txt     %s/combine.txt; '   %(subDirName)

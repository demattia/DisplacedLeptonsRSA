...
output_file_name = "SignalSampleInfoEffi_Neutralino_Combined.py"
output_file = open(output_file_name, "w")
output_file_name_acc = "SignalSampleInfoEffiOverAcc_Neutralino_Combined.py"
        ...
        if sRSA.MH == sTk.MH and sRSA.MX == sTk.MX:
            for ctauRSAIndex, ctauRSA in enumerate(sRSA.ctaus):
                ...
                for ctauTkIndex, ctauTk in enumerate(sTk.ctaus):
                    # print "ctauTk =", ctauTk
                    # Convert to int to avoid differences in float numbers
                    # such as 17.8*6 and 178*0.6 giving different results.
                    if int(1000000*ctauTk) == int(1000000*ctauRSA):
                        i = ctauTkIndex
                        break
                if i != -1:
                ...
                # Do not run anything that is not combined
                # else:
...
[MULTICRAB]
cfg=counting_job_Combined_Bayesian_neutralino/Muons_1000_148_20.0/TestGrid.cfg

[COMMON]
USER.ui_working_dir = counting_job_output_Combined_Bayesian_neutralino 
...
for subDirName in all_dirs:
    ...
    multicrab_file.write("""
["""+subDirName+"""] 
USER.script_exe = counting_job_Combined_Bayesian_neutralino/"""+subDirName+"""/TestGrid.sh 
USER.additional_input_files = combine, counting_job_Combined_Bayesian_neutralino/"""+subDirName+"""/combine.root 

...
class JobOptionsTk:
    ...
    def getDataRootFile(self, lepton):
        ...
        dataFile = "NONE"
        if lepton == "Muons":
            dataFile = "InputData2012_Tk/BackgroundStudiesFiles/Mass_removedLifetimeCuts_2muTrack.root"
        elif lepton == "Electrons":
            dataFile = "InputData2012_Tk/BackgroundStudiesFiles/Mass_removedLifetimeCuts_2eTrack.root"
        if not os.path.exists(dataFile):
            sys.exit("ERROR: Input data file %s not found !" % dataFile)
        return dataFile
...
def getSignalSampleInfoTk():
    ...
    if jobOpt.doNeutralinoLimits:
        ...
        execfile("%s/SignalSampleInfoEffi_Neutralino_Tk.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_Neutralino_Tk.py" % (dirScript))

    else:

        execfile("%s/SignalSampleInfoEffi_Tk.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_Tk.py" % (dirScript))

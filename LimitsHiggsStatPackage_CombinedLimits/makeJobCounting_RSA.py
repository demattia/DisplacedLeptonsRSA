...
from SignalSampleInfo_RSA import *  # Includes JobOptions class.
import sys
import os
import subprocess
...
if acceptanceOption == 1:
    if jobOpt.doNeutralinoLimits:
        jobDirName = "counting_job_RSA_%s_neutralino" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_RSA_%s_neutralino" % (jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_RSA_%s" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_RSA_%s" % (jobOpt.limitMethod)

elif acceptanceOption == 2:
    if jobOpt.doNeutralinoLimits:
        jobDirName = "counting_job_acc_RSA_%s_neutralino" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_RSA_%s_neutralino" % (jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_acc_RSA_%s" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_RSA_%s" % (jobOpt.limitMethod)
else:
    ...
    if s.acceptance == acceptanceOption:
        ...
        for l in leptons:
            ...
            for tIndex, t in enumerate(s.cTauScaleLoopValues):
                ...
                subDirName = '%s/%s_%i_%i_%s' % (jobDirName, l, s.MH, s.MX, t*10)
                ...
                cmd = 'cp combine_template_counting_RSA.txt     %s/combine.txt; ' % (subDirName)
                ...
                cmd = '$CMSSW_BASE/src/HiggsAnalysis/CombinedLimit/scripts/text2workspace.py %s/combine.txt -b -o %s/combine.root' % (
                ...
                if jobOpt.limitMethod == "CLs":
                    ...
                    cmd = '$CMSSW_BASE/src/HiggsAnalysis/CombinedLimit/test/makeGridUsingCrab.py combine.root %i %i -v3 -r -m 0 -O "-U --frequentist --rule CLs --testStat LEP" -n %i -j %i -t %i -T %i --out %s' % (
                        min_r, max_r, npoints, njobs, nevents, ntoys, crabFileName)
                    istat = subprocess.call(cmd, shell=True)
                    if istat != 0:
                        sys.exit("ERROR: whilst executing %s" % cmd)
                    ...
                    cmd = '$CMSSW_BASE/src/HiggsAnalysis/CombinedLimit/test/makeGridUsingCrab.py combine.root 0 0 -j 1 -t 1 --out %s' % (
                        crabFileName)
                    istat = subprocess.call(cmd, shell=True)
                    if istat != 0:
                        sys.exit("ERROR: whilst executing %s" % cmd)
                ...
                if USE_PBS:
                    ...
                    cmd = 'sed -i "s/scheduler = glite/scheduler = condor/g"   %s.cfg; ' % crabFileName
        ...
        for l in leptons:
            for tIndex, t in enumerate(s.cTauScaleLoopValues):
                ...
                if jobOpt.limitMethod == "CLs":
                    ...
                    F.write(
                        "CMSSW.output_file = higgsCombineTest.%s.mH0.root, higgsCombineTest.%s.mH0.123456.root, combine.root \n\n" % (

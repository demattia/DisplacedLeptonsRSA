...
if ( $inAcceptance eq 1 ) {
  $infileStem = "InputData2012_Tk//EfficiencyFiles/efficiencies_coll_Chi0ToNuLL";
} elsif ( $inAcceptance eq 2 ) {
  $infileStem = "InputData2012_Tk//EfficiencyFiles/efficiencyOverAcceptance/efficienciesOverAcceptance_coll_Chi0ToNuLL";
} else {
...
my %signalEffiSys_rel = ("e1"  => 0.101,
                         #"mu1" => 0.102); # with trigger
                         "mu1" => 0.081); # without trigger
...
my $ianfile;
if ($inAcceptance == 1) {
  $ianfile = "SignalSampleInfoEffi_Neutralino_Tk.py";
} elsif ($inAcceptance == 2) {
  $ianfile = "SignalSampleInfoEffiOverAcc_Neutralino_Tk.py";
}
...
foreach my $hmass (sort {$b <=> $a} keys %masses) {
	...
	next if (!$nominal_lifetimes{$pat});
        ...
        print IANFILE "#\n";
        print IANFILE "s = SignalSampleInfoTk(${hmass}, ${xmass}, ${nominal_lifetimes{$pat}}, $inAcceptance)\n";

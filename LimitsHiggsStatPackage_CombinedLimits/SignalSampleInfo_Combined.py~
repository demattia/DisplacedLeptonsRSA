...
class SignalSampleInfo:
    ...
    def enoughMCCombined(self, lepton, tIndex, loop=True):
        ...
        if loop:
            ...
            # print self.effis[lepton][tIndex]
...
def getSignalSampleInfoCombined():
    samples = []
    ...
    if jobOpt.doNeutralinoLimits:
        ...
        execfile("%s/SignalSampleInfoEffi_Neutralino_Combined.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_Neutralino_Combined.py" % (dirScript))

    else:

        execfile("%s/SignalSampleInfoEffi_Combined.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_Combined.py" % (dirScript))

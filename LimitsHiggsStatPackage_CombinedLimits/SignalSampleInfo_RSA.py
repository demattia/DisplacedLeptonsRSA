...
class jobObtionsRSA:
    ...
    def getDataTxtFile(self, lepton):
        dataFile = "NONE"
        if lepton == "Muons":
            dataFile = "InputData2012_Tk/MassFiles/muons/Data_Mu_All_coll.txt"
        elif lepton == "Electrons":
            dataFile = "InputData2012_Tk/MassFiles/electrons/Data_Photon_All_coll.txt"
    ...
    def getDataRootFile(self, lepton):
        ...
        dataFile = "NONE"
        if lepton == "Muons":
            dataFile = "InputData2012_RSA/BackgroundStudiesFiles/Mass_removedLifetimeCuts_2muTrack.root"
        elif lepton == "Electrons":
            dataFile = "InputData2012_RSA/BackgroundStudiesFiles/Mass_removedLifetimeCuts_2eTrack.root"
        if not os.path.exists(dataFile):
            sys.exit("ERROR: Input data file %s not found !" % dataFile)
        return dataFile
...
class SignalSampleInfoRSA:
    def __init__(self,
        ...
        self.bkg_relerr["Electrons"] = 0.11
        self.bkg_relerr["Muons"] = 0.58
        #
    ...
    def enoughMC(self, lepton, tIndex, loop=True):
        ...
        if loop:
            OK = self.effis[lepton][tIndex] > jobOpt.minPlotEff and self.effi_relerrs[lepton][
                                                                        tIndex] < jobOpt.maxPlotRelEffErr
...
def getSignalSampleInfoRSA():
    ...
    if jobOpt.doNeutralinoLimits:
        ...
        execfile("%s/SignalSampleInfoEffi_Neutralino_RSA.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_Neutralino_RSA.py" % (dirScript))

    else:

        execfile("%s/SignalSampleInfoEffi_RSA.py" % (dirScript))
        execfile("%s/SignalSampleInfoEffiOverAcc_RSA.py" % (dirScript))

...
if ( $inAcceptance eq 1 ) {
  $infileStem = "InputData2012//EfficiencyFiles/efficiencies_coll_Chi0ToNuLL";
} elsif ( $inAcceptance eq 2 ) {
  $infileStem = "InputData2012//EfficiencyFiles/efficiencyOverAcceptance/efficienciesOverAcceptance_coll_Chi0ToNuLL";
} else {
...
my %nominal_lifetimes = ("120_48" => 16.5,
		         "350_148" => 17.3,
		         "1000_148" => 6.,
		         "1500_494" => 16.0);
...
while (my $line = <INMASSFILE>) {
    ...
    if ($line =~ /Chi0ToNuLL/) {
	    ...
	    foreach my $xmass (@xmasses) {	    
	        my $massString = "_MSquark${hmass}_MChi${xmass}";
                if ($line =~ $massString) {
...
foreach my $hmass (keys %masses) {
    ...
    foreach my $xmass (@xmasses) {	    
        if (!$massRes{"mu"}{1}{$hmass}{$xmass}) {
            # IRT - Expected error - no mass resolutions calculated yet for neutralinos.
...
foreach my $channel (sort keys %channels) {
	...
	foreach my $xmass (@xmasses) {
		...
		# Instead use the one with the smallest PDG code of the exotic (works because many of those with lifetime scale 
		# factor > 1 were rejected above.
		if (!$pdgused{$channel}{$ctauFactor}{$hmass}{$xmass} || $pdg < $pdgused{$channel}{$ctauFactor}{$hmass}{$xmass}) {
			...
			# --- Replace normal efficiencies by truncated efficiencies in this case.
                        # (Should be OK, as truncation is at 6*orig_ctau = ~120 cm)
		    ...
		    # --- Use normal efficiencies in this case.
		    $eff{$channel}{$ctauFactor}{$hmass}{$xmass} = $effi;
		    ...
		    print "SHIT ",$ctauFactOrig," ",$hmass," ",$xmass," ",$channel," ",$effi," ",$staterr," ",$toterr,"\n";
		    ...
		    if ($efferr{$channel}{$ctauFactor}{$hmass}{$xmass} < 0) {
		       # Efficiency is sometimes -1 for dicandidate case, just through lack of stats, so don't print error in that case.
   		       if ($channel eq "mu1" or $channel eq "e1") {
			   print "ERROR: Efficiency uncertainty is negative for channel=",$channel,", masses=(",$hmass,",",$xmass,"), lifetime factor=",$ctauFactor," \n";
			   exit 1;
		       }
		    }
		}
...
my $ianfile;
if ($inAcceptance == 1) {
  $ianfile = "SignalSampleInfoEffi_Neutralino.py";
} elsif ($inAcceptance == 2) {
  $ianfile = "SignalSampleInfoEffiOverAcc_Neutralino.py";
}
...
foreach my $hmass (sort {$b <=> $a} keys %masses) {
	...
	print IANFILE "] )\n";
        ...
        # Write out signal mass resolution data. # IRT commented out as not available.

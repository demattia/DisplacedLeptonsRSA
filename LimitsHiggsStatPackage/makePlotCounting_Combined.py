...
#
# Call makePlotCounting.py with argument 1 (2) to get limits on sigma*BR (sigma*BR*acceptance).
# Or argument 11 (12) to simply put limit on mean number of signal events passing cuts.
###############################################################################################

from SignalSampleInfo_Combined import *
from convertLimits_rootToTxt_Combined import *
import sys
import os
...
#=== Lifetime scaling factors bigger than this will not be shown in the plot.
biggestScalingFactor = 1000
...
if acceptanceOption % 10 == 1:
    if jobOpt.doNeutralinoLimits:
        print "Neutralino"
        jobDirName = "counting_job_Combined_%s_neutralino" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_Combined_%s_neutralino" % (jobOpt.limitMethod)
        resultDirName = "counting_result_Combined_%s_neutralino" % (jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_Combined_%s" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_Combined_%s" % (jobOpt.limitMethod)
        resultDirName = "counting_result_Combined_%s" % (jobOpt.limitMethod)

elif acceptanceOption % 10 == 2:
    if jobOpt.doNeutralinoLimits:
        print "Neutralino"
        jobDirName = "counting_job_acc_Combined_%s_neutralino" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_Combined_%s_neutralino" % (jobOpt.limitMethod)
        resultDirName = "counting_result_acc_Combined_%s_neutralino" % (jobOpt.limitMethod)
    else:
        jobDirName = "counting_job_acc_Combined_%s" % (jobOpt.limitMethod)
        jobOutName = "counting_job_output_acc_Combined_%s" % (jobOpt.limitMethod)
        resultDirName = "counting_result_acc_Combined_%s" % (jobOpt.limitMethod)
else:
        ...
        for s in samples:
            ...
            if s.acceptance == acceptanceOption % 10:
                ...
                for tIndex, t in enumerate(s.cTauScaleLoopValues):
                    ...
                    if not s.enoughMCCombined(l, tIndex, jobOpt.loopOverCTau):
                        # print "NOT ENOUGH MC for", t
                        continue  # Skip this point if not enough MC statistics.

                    # Limit the range of the limits plotted. This is because the track-based limits
                    # are only available up to a scale factor of 1000 and the limits are not combined
                    # after this point.
                    if t > biggestScalingFactor:
                        continue
                    ...
                    job_name = '%s/%s/%s_%i_%i_%s/' % (start_dir, jobDirName, l, s.MH, s.MX, t)
                    # Output directory from CRAB jobs
                    job_output_name = '%s/%s/%s_%i_%i_%s/res/' % (start_dir, jobOutName, l, s.MH, s.MX, t)
                    ...
                    cmd = "cp -f %s/combine.root %s/combine.root" % (job_name, result_name)
                    ...
                    if jobOpt.limitMethod == "CLs":
                        ...
                        cmd = "combine combine.root -v2 -m 0 -M %s -U --fullGrid --frequentist --rule CLs --testStat LEP --grid=sum.root" % (
                            limitMethodCall)
                        print cmd, "\n"
                        ...
                        for frac in jobOpt.fracs:
                            ...
                            cmd = "combine combine.root -v2 -m 0 -M %s -U --fullGrid --frequentist --rule CLs --testStat LEP --expectedFromGrid %.3f --grid=sum.root" % (
                                limitMethodCall, frac)
                            print cmd, "\n"
                            istat = subprocess.call(cmd, shell=True, stdout=cfile)
                            if istat != 0:
                                sys.exit("ERROR: whilst executing %s" % cmd)
                        ...
                        # Get observed limit (name includes .mH0_ followed by some numbers added by CRAB).
                        cmd = "cp %s/higgsCombineTest.*.mH0_*.root higgsCombineTest.%s.mH0.root" % (
                            job_output_name, limitMethodCall)
                        print cmd, "\n"
                        istat = subprocess.call(cmd, shell=True, stdout=cfile)
                        if istat != 0:
                            sys.exit("ERROR: whilst executing %s" % cmd)

                        # Get expected limit (name includes .mH0. followed by the random seed used for the toys followed by CRAB numbers).
                        cmd = "cp %s/higgsCombineTest.*.mH0.123456*.root higgsCombineTest.%s.mH0.quantAll.root" % (
                            job_output_name, limitMethodCall)
                        print cmd, "\n"
                        istat = subprocess.call(cmd, shell=True, stdout=cfile)
                        if istat != 0:
                    ...
                    if jobOpt.limitMethod == "CLs":
                        ...
                        for frac in jobOpt.fracs:
                            expLimitsRootFiles.append(
                                "higgsCombineTest.%s.mH0.quant%.3f.root" % (limitMethodCall, frac))
                    elif jobOpt.limitMethod == "Bayesian":
                        # expected limit for each toy, all in one file.
                        expLimitRootFiles.append("higgsCombineTest.%s.mH0.quantAll.root" % (limitMethodCall))

                    subtractControl = True  # efficiency is that in signal region minus that in control region.
                    convertLimits_rootToTxt(s, l, tIndex, obsLimitRootFile, expLimitRootFiles, acceptanceOption,
                                            subtractControl, txt_name)
...
if jobOpt.doNeutralinoLimits:
    ...
    for l in leptons:
        ...
        cmd += 'python %s/PlotLimitsLifetimeExpected_Combined.py %s %i %i %i' % (
            start_dir, l, not jobOpt.doNeutralinoLimits, -1, getSigmaBRacc)
        print '-- ', cmd
        istat = subprocess.call(cmd, shell=True)
        if istat != 0:
            sys.exit("ERROR: whilst executing %s" % cmd)
    ...
    for mH in higgs_masses:
        ...
        for l in leptons:
            ...
            cmd = 'cp ../../LimitsRooStats/tdrstyle.C . ;'
            cmd += 'python %s/PlotLimitsLifetimeExpected_Combined.py %s %i %i %i' % (
                start_dir, l, not jobOpt.doNeutralinoLimits, mH, getSigmaBRacc)
            print '-- ', cmd
            istat = subprocess.call(cmd, shell=True)
            if istat != 0:
                sys.exit("ERROR: whilst executing %s" % cmd)

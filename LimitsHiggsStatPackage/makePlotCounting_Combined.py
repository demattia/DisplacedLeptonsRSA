              ...
              if s.acceptance == acceptanceOption%10: 
                ...
                for tIndex, t in enumerate(s.cTauScaleLoopValues):
                       ...
                       if jobOpt.limitMethod == "CLs":
                            ...
                            cmd = "combine combine.root -v2 -m 0 -M %s -U --fullGrid --frequentist --rule CLs --testStat LEP --grid=sum.root"  %(limitMethodCall)
                            print cmd,"\n"
                            ...
                            for frac in jobOpt.fracs:
                                ...
                                cmd = "combine combine.root -v2 -m 0 -M %s -U --fullGrid --frequentist --rule CLs --testStat LEP --expectedFromGrid %.3f --grid=sum.root"   %(limitMethodCall, frac)
                                print cmd,"\n"
                                istat = subprocess.call(cmd, shell=True, stdout=cfile)
                                if istat != 0:
                                    sys.exit("ERROR: whilst executing %s" %cmd)
                            ...
                            cmd = "cp %s/higgsCombineTest.*.mH0_*.root higgsCombineTest.%s.mH0.root"  %(job_output_name, limitMethodCall)
                            print cmd,"\n"
                            istat = subprocess.call(cmd, shell=True, stdout=cfile)
                            if istat != 0:
                                sys.exit("ERROR: whilst executing %s" %cmd)
                            ...
                            cmd = "cp %s/higgsCombineTest.*.mH0.123456*.root higgsCombineTest.%s.mH0.quantAll.root"  %(job_output_name, limitMethodCall)
                            print cmd,"\n"
                            istat = subprocess.call(cmd, shell=True, stdout=cfile)
                            if istat != 0:
                                sys.exit("ERROR: whilst executing %s" %cmd)
...
if jobOpt.doNeutralinoLimits:
       ...
       for l in leptons:
              ...
              cmd += 'python %s/PlotLimitsLifetimeExpected_Combined.py %s %i %i %i'  %(start_dir, l, not jobOpt.doNeutralinoLimits, -1, getSigmaBRacc)
              print '-- ',cmd
              istat = subprocess.call(cmd, shell=True)
              if istat != 0:
                     sys.exit("ERROR: whilst executing %s" %cmd)
   ...
   for mH in higgs_masses:
       ...
       for l in leptons:
              ...
              getSigmaBRacc = (acceptanceOption%10 == 2)
              cmd = 'cp ../../LimitsRooStats/tdrstyle.C . ;'
              cmd += 'python %s/PlotLimitsLifetimeExpected_Combined.py %s %i %i %i'  %(start_dir, l, not jobOpt.doNeutralinoLimits, mH, getSigmaBRacc)
              print '-- ',cmd
              istat = subprocess.call(cmd, shell=True)
              if istat != 0:
                     sys.exit("ERROR: whilst executing %s" %cmd)

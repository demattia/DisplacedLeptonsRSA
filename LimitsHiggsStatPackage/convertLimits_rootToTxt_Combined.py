...
def convertLimits_rootToTxt(sample, lepton, tIndex, obsLimitRootFile, expLimitRootFiles, acceptanceOption,
                            subtractControl, txtDirName):
    # -----------------------------------------------------------------------------------------------------
    ...
    for index in indexLoop:
        ...
        if eff > jobOpt.minPlotEff and sample.effi_relerrs[lepton][index] < jobOpt.maxPlotRelEffErr:
            ...
            # Note masses and c*tau scale factor in output .txt file.
            resultListForPaul = []
            resultListForPaul.append(sample.MH)
            resultListForPaul.append(sample.MX)
            resultListForPaul.append(sample.ctauScales[index])
            # Also note relative uncertainty on efficiency, so know how reliable this point is.
            resultListForPaul.append(sample.effi_relerrs[lepton][index])
            ...
            for result in resultList:
                if result > 0:
                    ...
                    if acceptanceOption < 10:
                        ...
                        # This is worse, because some exotics end up in same event.
                    ...
                    if jobOpt.limitMethod == "CLs" and result > 0.8 * jobOpt.maxPredictedLimit:
                        sys.exit(
                            "ERROR: Parameter maxPredictedLimit = %f in SignalSampleInfo.py is too small. Please increase to at least %f" % (
                                jobOpt.maxPredictedLimit, 1.5 * result))
                else:
                ...
                print "Limit on signal cands = ", result, " Limit on sigma*BR = ", lim0, "(", lim1, ") for small (large) BR; effi = ", eff, "; lumi = ", jobOpt.getLumi(
                    lepton)

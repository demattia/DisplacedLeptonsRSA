...
{
  // Store in a TProfile
  TH1F *cutFlow = new TH1F("cutFlow","Cut flow for final set of analysis cuts",16,0.5,16.5);
  ...
  // Store in a TProfile
  TH1F *cutFlowColl = new TH1F("cutFlowColl","Cut flow for final set of analysis cuts",16,0.5,16.5);
  cutFlowAxis = cutFlowColl->GetXaxis();

  // Store number of signal MC exotics before cut flow in separate histogram
  if ( isSignalMC_ ) {
    TH1F *cutFlowColl_preselection = new TH1F("cutFlowColl_preselection","Cut flow for preselection of analysis cuts",16,0.5,16.5);
                                          ...
                                          double & mass,Histograms & h, bool fillCorrectedTextFiles, const DileptonAnalysisCuts & dileptonCuts_finalColl_muTrack )
{
  ...
  finalCollCuts_removedLifetime.passMaxLxySig = true;
  finalCollCuts_removedLifetime.passMinLxySig = true;
  finalCollCuts_removedLifetime.passTrackRejection = true;
